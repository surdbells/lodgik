...
#[ORM\Entity] #[ORM\Table(name:'offboarding_checklists')]
class OffboardingChecklist { use HasUuid,HasTenant;
    ...
    #[ORM\Column(name:'assigned_name',type:Types::STRING,length:150,nullable:true)] private ?string $assignedName=null;
    ...
    public function setAssignedName(?string $v):void{$this->assignedName=$v;}
    ...
    public function toArray():array{return['id'=>$this->getId(),'employee_id'=>$this->employeeId,'title'=>$this->title,'category'=>$this->category,'description'=>$this->description,'is_mandatory'=>$this->isMandatory,'assigned_to'=>$this->assignedTo,'assigned_name'=>$this->assignedName,'due_date'=>$this->dueDate?->format('Y-m-d'),'completed_at'=>$this->completedAt?->format('Y-m-d H:i:s'),'completed_by'=>$this->completedBy,'notes'=>$this->notes,'is_completed'=>$this->isCompleted()];}
}
