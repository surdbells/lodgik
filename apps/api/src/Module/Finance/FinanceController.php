...
final class FinanceController
{
    ...
    // Performance Reviews
    public function listReviews(Request $req, Response $res): Response { $p = $req->getQueryParams(); $pid = $p['property_id'] ?? $req->getAttribute('auth.property_id') ?? ''; return $this->json($res, ['success' => true, 'data' => $this->svc->listReviews($pid, $p['employee_id'] ?? null)]); }
    public function createReview(Request $req, Response $res): Response { $d = $this->body($req); $pid = $d['property_id'] ?? $req->getAttribute('auth.property_id') ?? ''; if (empty($d['employee_id'])||empty($d['employee_name'])||empty($d['period'])||!isset($d['year'])||!isset($d['overall_rating'])) return $this->json($res, ['success'=>false,'message'=>'employee_id, employee_name, period, year, overall_rating required'], 422); $r = $this->svc->createReview($pid, $d['employee_id'], $d['employee_name'], $req->getAttribute('auth.user_id'), $d['reviewer_name'] ?? 'Manager', $d['period'], (int)$d['year'], (int)$d['overall_rating'], $req->getAttribute('auth.tenant_id'), $d); return $this->json($res, ['success' => true, 'data' => $r->toArray()], 201); }
