...
final class OtaController
{
    ...
    public function channelRevenue(Request $req, Response $res): Response { $p = $req->getQueryParams(); return $this->json($res, ['success' => true, 'data' => $this->svc->getRevenueByChannel($req->getAttribute('auth.tenant_id'), $p['from'] ?? date('Y-01-01'), $p['to'] ?? date('Y-m-d'))]); }
}
