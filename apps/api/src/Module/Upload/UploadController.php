...
final class UploadController
{
    ...
    // ─────────────────────────────────────────────────────────────
    // GET /storage/{path:.+}  — serve stored file
    // ─────────────────────────────────────────────────────────────

    /**
     * Stream a stored file to the browser.
     *
     * Handles URLs generated by FileStorageService::publicUrl() for the local
     * driver, e.g.:
     *   https://api.lodgik.co/storage/document/abc123.pdf
     *   https://api.lodgik.co/storage/tenants/hotel-one/branding/logo_abc.png
     *
     * Security:
     *   - Path must begin with an allowed top-level context
     *   - realpath() check ensures the resolved file is inside STORAGE_PATH
     *   - No auth required — the opaque random filename is the access control
     */
    public function serveFile(Request $req, Response $res, array $args): Response
    {
        $relPath = trim($args['path'] ?? '', '/');

        // ── Basic sanity ─────────────────────────────────────────
        if ($relPath === '') {
            return $this->fileNotFound($res);
        }

        // ── Allowed top-level contexts ───────────────────────────
        $topLevel = explode('/', $relPath)[0];
        $allowed  = ['kyc', 'document', 'avatar', 'resource', 'other', 'binary', 'tenants'];
        if (!in_array($topLevel, $allowed, true)) {
            return $this->fileNotFound($res);
        }

        // ── Resolve absolute path ────────────────────────────────
        $storagePath = rtrim($_ENV['STORAGE_PATH'] ?? (dirname(__DIR__, 3) . '/storage'), '/');
        $filePath    = $storagePath . '/' . $relPath;

        // realpath() resolves symlinks and `..` — if it differs from what we
        // expect, a traversal attempt was made.
        ...
        if ($stream === false) {
            $body = $res->getBody();
            $body->write(json_encode(['success' => false, 'message' => 'Could not read file'], JSON_THROW_ON_ERROR));
            return $res->withStatus(500)->withHeader('Content-Type', 'application/json');
        }

        // Inline for viewable types (browser renders PDF/image directly);
        // attachment (force-download) for everything else.
        $inlineTypes = ['application/pdf', 'image/jpeg', 'image/png', 'image/gif',
        ...
        /** @var \Slim\Psr7\Stream $body */
        $psrBody = new \Slim\Psr7\Stream($stream);
        ...
        return $res
            ...
            ->withHeader('Content-Disposition',
                "{$disposition}; filename=\"{$basename}\"; filename*=UTF-8''{$encoded}")
            ->withHeader('Cache-Control', 'private, max-age=3600')
            ->withHeader('X-Content-Type-Options', 'nosniff')
            ->withHeader('Access-Control-Allow-Origin', '*');
    }

    /** Return a clean 404 JSON response (no stack trace, no file path leakage). */
    private function fileNotFound(Response $res): Response
    {
