...
@Component({
  ...
  template: `
    <ui-page-header title="Subscription Plans" subtitle="Manage pricing plans with module bundles">
      <button class="px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700" (click)="toggleCreate()">
      ...
      <div class="bg-white rounded-lg border border-gray-200 p-5 mb-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
          <div><label class="block text-xs font-medium text-gray-600 mb-1">Plan Name</label>
            <input [(ngModel)]="form.name" placeholder="Professional" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"></div>
          <div><label class="block text-xs font-medium text-gray-600 mb-1">Tier</label>
            <select [(ngModel)]="form.tier" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
          ...
          <div><label class="block text-xs font-medium text-gray-600 mb-1">Monthly Price (NGN)</label>
            <input [(ngModel)]="form.monthly_price" type="number" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"></div>
          <div><label class="block text-xs font-medium text-gray-600 mb-1">Annual Price (NGN)</label>
            <input [(ngModel)]="form.annual_price" type="number" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"></div>
        </div>

        <!-- Limits -->
        <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-4">
          <div><label class="block text-xs font-medium text-gray-600 mb-1">Max Rooms</label>
            <input [(ngModel)]="form.max_rooms" type="number" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"></div>
          <div><label class="block text-xs font-medium text-gray-600 mb-1">Max Staff</label>
            <input [(ngModel)]="form.max_staff" type="number" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"></div>
          <div><label class="block text-xs font-medium text-gray-600 mb-1">Max Properties</label>
            <input [(ngModel)]="form.max_properties" type="number" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"></div>
          <div><label class="block text-xs font-medium text-gray-600 mb-1">Trial Days</label>
            <input [(ngModel)]="form.trial_days" type="number" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm"></div>
        </div>
        ...
        <div class="mb-4">
          <div class="flex items-center justify-between mb-2">
            ...
            <div class="flex gap-2">
              <button (click)="selectAllModules()" class="text-xs text-blue-600 hover:underline">Select All</button>
              ...
              <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-1">
                @for (m of cat.modules; track m.module_key) {
                  <label class="flex items-center gap-2 px-2 py-1.5 rounded text-xs cursor-pointer hover:bg-gray-50"
                         [class.bg-blue-50]="form.included_modules.includes(m.module_key)"
        ...
        <div class="flex gap-2">
          <button (click)="savePlan()" class="px-4 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700">
