...
@Component({
  ...
  template: `
    <ui-page-header title="Guest Chat" subtitle="Respond to guest messages across departments"></ui-page-header>
      ...
      <div class="w-80 bg-white border rounded-lg flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto">
          @for (chat of filteredChats(); track chat.booking_id) {
            <div (click)="selectChat(chat)" class="p-3 border-b cursor-pointer hover:bg-gray-50" [class.bg-blue-50]="selectedBookingId === chat.booking_id">
        ...
        @if (selectedBookingId) {
            ...
            <div class="flex gap-1">
              @for (dept of departments; track dept.value) {
                <button (click)="replyDepartment = dept.value"
                  [class]="replyDepartment === dept.value ? 'px-2 py-1 text-xs bg-blue-600 text-white rounded' : 'px-2 py-1 text-xs border rounded hover:bg-gray-50'">
                  {{ dept.label }}
                </button>
              }
            </div>
          ...
          <div class="flex-1 overflow-y-auto p-4 space-y-2 bg-gray-50" #messageContainer>
            @for (m of messages(); track m.id) {
              <div [class]="m.sender_type === 'staff' ? 'flex justify-end' : 'flex justify-start'">
                <div [class]="m.sender_type === 'staff' ? 'bg-blue-600 text-white rounded-2xl rounded-br-sm px-4 py-2 max-w-md' : 'bg-white border rounded-2xl rounded-bl-sm px-4 py-2 max-w-md'">
                  ...
                  <div [class]="m.sender_type === 'staff' ? 'text-xs text-blue-200 text-right mt-1' : 'text-xs text-gray-400 mt-1'">{{ m.created_at?.substring(11, 16) }}</div>
                </div>
              </div>
            }
          </div>
          ...
          <div class="p-3 border-t flex gap-2">
            <input type="text" [(ngModel)]="newMessage" placeholder="Type a reply..." (keydown.enter)="send()"
              class="flex-1 px-4 py-2 border rounded-full text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            <button (click)="send()" [disabled]="!newMessage.trim()" class="px-5 py-2 bg-blue-600 text-white rounded-full text-sm font-medium hover:bg-blue-700 disabled:opacity-50">Send</button>
          </div>
        } @else {
