    ...
    <div class="min-h-screen bg-gradient-to-br from-blue-900 via-blue-800 to-indigo-900 flex items-center justify-center p-4">
      <div class="w-full max-w-md">
        <div class="text-center mb-8">
          ...
          <p class="text-blue-200 mt-1">Hotel Management Platform</p>
        </div>
        <div class="bg-white rounded-xl shadow-2xl p-8">
          ...
          <form (ngSubmit)="login()" class="space-y-4">
            ...
            <button type="submit" [disabled]="loading()"
                    class="w-full py-2.5 bg-blue-700 text-white rounded-lg text-sm font-medium hover:bg-blue-600 disabled:opacity-50 transition-colors">
              {{ loading() ? 'Signing in...' : 'Sign in' }}
            </button>
          </form>
          <p class="mt-4 text-center text-sm text-gray-500">
            Don't have an account? <a routerLink="/register" class="text-blue-600 hover:underline">Register your hotel</a>
          </p>
        </div>
      </div>
    </div>
