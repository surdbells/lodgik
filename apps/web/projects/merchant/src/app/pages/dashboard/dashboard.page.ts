...
@Component({
  ...
  template: `
    <ui-page-header title="Dashboard" subtitle="Welcome to your Merchant Portal"></ui-page-header>
    <ui-loading [loading]="loading()"></ui-loading>
    @if (!loading()) {
      <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <ui-stats-card label="Active Hotels" [value]="dash().hotels?.total || 0" icon="🏨"></ui-stats-card>
        <ui-stats-card label="Total Earned" [value]="'₦' + (+dash().earnings?.total_earned || 0).toLocaleString()" icon="💰"></ui-stats-card>
        <ui-stats-card label="Pending Commission" [value]="'₦' + (+dash().earnings?.pending || 0).toLocaleString()" icon="⏳"></ui-stats-card>
        <ui-stats-card label="Total Paid" [value]="'₦' + (+dash().earnings?.paid || 0).toLocaleString()" icon="✅"></ui-stats-card>
      </div>
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="bg-white rounded-lg border border-gray-200 p-5 space-y-2">
          ...
          <a routerLink="/hotels" class="block w-full py-2 px-3 bg-emerald-50 text-emerald-700 text-sm rounded-lg hover:bg-emerald-100 text-center">+ Register Hotel</a>
          <a routerLink="/leads" class="block w-full py-2 px-3 bg-blue-50 text-blue-700 text-sm rounded-lg hover:bg-blue-100 text-center">+ New Lead</a>
          ...
          <div class="flex gap-2 mt-3 pt-3 border-t border-gray-100">
            ...
            <div class="flex-1 text-center"><div class="text-lg font-bold text-blue-500">{{ dash().unread_notifications || 0 }}</div><div class="text-[10px] text-gray-500">Unread</div></div>
          </div>
        </div>
      </div>
    }
