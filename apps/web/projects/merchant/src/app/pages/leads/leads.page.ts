...
@Component({
  ...
  template: `
    <ui-page-header title="Leads" subtitle="Manage your sales pipeline">
      <button (click)="showForm.set(true)" class="px-4 py-2 bg-emerald-600 text-white text-sm rounded-lg hover:bg-emerald-700">+ New Lead</button>
    </ui-page-header>
    ...
    @if (showForm()) {
      <div class="bg-white rounded-lg border p-5 mb-6">
        ...
        <div class="flex gap-2 mt-4">
          <button (click)="create()" class="px-4 py-2 bg-emerald-600 text-white text-sm rounded-lg">Save Lead</button>
    ...
    @if (!loading()) {
      ...
      <div class="bg-white rounded-lg border overflow-hidden">
        <table class="w-full text-sm">
          <thead class="bg-gray-50 text-xs text-gray-500 uppercase"><tr><th class="px-4 py-2 text-left">Hotel</th><th class="px-4 py-2 text-left">Contact</th><th class="px-4 py-2 text-left">Location</th><th class="px-4 py-2">Rooms</th><th class="px-4 py-2">Status</th><th class="px-4 py-2">Date</th><th class="px-4 py-2"></th></tr></thead>
          <tbody class="divide-y divide-gray-100">
            @for (l of leads(); track l.id) {
              <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-2 text-gray-500">{{ l.created_at | date:'shortDate' }}</td>
                <td class="px-4 py-2">
                  @if (l.status !== 'converted' && l.status !== 'lost') {
                    <button (click)="convert(l.id)" class="text-emerald-600 hover:underline text-xs mr-2">Convert</button>
                  }
                </td>
              </tr>
...
export class LeadsPage implements OnInit {
  ...
  loading = signal(true); showForm = signal(false); leads = signal<any[]>([]); filterStatus = signal('');
  form: any = { hotel_name: '', contact_name: '', contact_phone: '', contact_email: '', location: '', rooms_estimate: 0, notes: '' };
  filters = [{ label: 'All', value: '' }, { label: 'Lead', value: 'lead' }, { label: 'Contacted', value: 'contacted' }, { label: 'Converted', value: 'converted' }];
  ...
  create(): void {
    this.api.createLead(this.form).subscribe({
      next: () => { this.toast.success('Lead created'); this.showForm.set(false); this.form = { hotel_name: '', contact_name: '', contact_phone: '', contact_email: '', location: '', rooms_estimate: 0, notes: '' }; this.load(); },
      error: (e: any) => this.toast.error(e?.error?.error || 'Failed')
    });
  }
  convert(id: string): void { this.api.convertLead(id, {}).subscribe({ next: () => { this.toast.success('Lead converted to hotel'); this.load(); }, error: (e: any) => this.toast.error(e?.error?.error || 'Failed') }); }
}
